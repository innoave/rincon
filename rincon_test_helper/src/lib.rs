...
pub fn system_datasource() -> DataSource {
    ...
    DataSource::from_str(&db_url).unwrap()
}
    ...
    CleanUp: FnOnce(JsonHttpConnection, &mut Core) -> (),
{
    ...
    let system_ds = DataSource::from_str(&db_url).unwrap();
    ...
    Test: FnOnce(JsonHttpConnector, Core) -> () + panic::UnwindSafe,
{
    ...
    let result = panic::catch_unwind(|| {
        let core = Core::new().unwrap();
        let user_ds = DataSource::from_str(&db_url)
            .unwrap()
    ...
    Test: FnOnce(JsonHttpConnection, &mut Core) -> () + panic::UnwindSafe,
{
    ...
    core.run(user_conn.execute(CreateCollection::documents_with_name(collection)))
        .expect(&format!(
            "Error on creating document collection: {}",
            collection
        ));
    ...
    Test: FnOnce(JsonHttpConnection, &mut Core) -> () + panic::UnwindSafe,
{
    ...
    core.run(user_conn.execute(CreateCollection::edges_with_name(collection)))
        .expect(&format!(
            "Error on creating edge collection: {}",
            collection
        ));
...
fn release_file_lock() {
    let mut lock_file = env::current_dir().unwrap();
    lock_file.push(LOCK_FILE);
    fs::remove_file(&lock_file).expect(&format!("Error deleting lock file: {:?}", &lock_file));
}

#[allow(dead_code)]
fn is_database_existing(database: &str, conn: &JsonHttpConnection, core: &mut Core) -> bool {
    let db_list = core
        .run(conn.execute(ListAccessibleDatabases::new()))
        .expect(&format!(
            "Could not get list of accessible databases for connection: {:?}",
            conn
        ));
    ...
    Db: Into<String>,
{
    ...
    let created = core
        .run(sys_conn.execute(CreateDatabase::new(new_database)))
        .unwrap();
...
{
    let _ = core
        .run(sys_conn.execute(DropDatabase::with_name(database)))
        .unwrap();
    let _ = core
        .run(sys_conn.execute(DeleteUser::with_name(user)))
        .unwrap();
}
