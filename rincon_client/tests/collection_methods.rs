...
fn drop_collection_should_return_the_id_of_the_dropped_collection() {
    arango_test_with_user_db("test_coll_user3", "test_coll_db31", |conn, ref mut core| {
        let collection1 = core
            .run(conn.execute(CreateCollection::with_name("test_collection1")))
            .unwrap();
        let _ = core
            .run(conn.execute(CreateCollection::with_name("test_collection2")))
            .unwrap();
...
fn list_collections_should_return_two_collections() {
    arango_test_with_user_db("test_coll_user4", "test_coll_db41", |conn, ref mut core| {
        let _ = core
            .run(conn.execute(CreateCollection::documents_with_name("test_collection1")))
            .unwrap();
        let _ = core
            .run(conn.execute(CreateCollection::edges_with_name("test_collection2")))
            .unwrap();
        ...
        let collection1 = collections
            .iter()
            .find(|coll| coll.name() == "test_collection1")
            .unwrap();
        ...
        let collection2 = collections
            .iter()
            .find(|coll| coll.name() == "test_collection2")
            .unwrap();
    ...
    arango_test_with_user_db("test_coll_user7", "test_coll_db71", |conn, ref mut core| {
        ...
        match result {
            Err(Error::Method(error)) => {
                ...
                assert_eq!(
                    "unknown collection 'test_collection_not_existing'",
                    error.message()
                );
            },
    ...
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            let _ = core
                ...
                .unwrap();
            let original = core
                .run(conn.execute(GetCollectionProperties::with_name("test_collection1")))
                .unwrap();
...
fn rename_collection_to_new_name() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            let method =
                RenameCollection::with_name("test_collection1").to_name("test_collection_renamed");
            let work = conn.execute(method);
            let updated = core.run(work).unwrap();
...
fn rename_collection_to_empty_name() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            let method = RenameCollection::with_name("test_collection1").to_name("");
            let work = conn.execute(method);
            let result = core.run(work);
