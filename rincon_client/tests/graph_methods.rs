...
fn create_graph() {
    arango_test_with_user_db(
        ...
        "test_graph_db10",
        |conn, ref mut core| {
            let edge_defs = vec![EdgeDefinition::new(
                "UsersInGroups",
                vec!["Users".to_owned()],
                vec!["Groups".to_owned()],
            )];
...
fn create_smart_graph() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            new_graph
                .options_mut()
                .set_smart_graph_attribute("knows".to_owned());
...
fn list_graphs() {
    arango_test_with_user_db(
        ...
        "test_graph_db40",
        |conn, ref mut core| {
            let edge_defs1 = vec![EdgeDefinition::new(
                "UsersInGroups",
                vec!["Users".to_owned()],
                vec!["Groups".to_owned()],
            )];
            ...
            let created1 = core
                .run(conn.execute(CreateGraph::new(new_graph1)))
                .unwrap();
            assert_eq!("_graphs/test_graph1".to_owned(), created1.id().to_string());

            let edge_defs2 = vec![EdgeDefinition::new(
                "UsersKnowsUsers",
                vec!["Users".to_owned()],
                vec!["Users".to_owned()],
            )];
            ...
            let created2 = core
                .run(conn.execute(CreateGraph::new(new_graph2)))
                .unwrap();
    ...
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            assert!(
                graph
                    .orphan_collections()
                    .contains(&"other_vertices".to_owned())
            );
        },
    );
}
...
fn remove_vertex_collection() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            #[cfg(not(feature = "enterprise"))]
            let new_graph = NewGraph::new("test_graph1", edge_defs)
                .with_orphan_collections(vec!["add_ons".to_owned(), "spare".to_owned()]);
            #[cfg(feature = "enterprise")]
            let new_graph = NewGraph::new("test_graph1", edge_defs, false)
                .with_orphan_collections(vec!["add_ons".to_owned(), "spare".to_owned()]);
            let created = core.run(conn.execute(CreateGraph::new(new_graph))).unwrap();
...
fn insert_vertex() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            let city_doc = NewDocument::from(City {
                name: "New Orleans".to_owned(),
            });
...
fn add_edge_definition() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            let works_in = EdgeDefinition::new(
                "works_in",
                vec!["female".to_owned(), "male".to_owned()],
                vec!["city".to_owned()],
            );
...
fn remove_edge_definition() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            assert_eq!("social", graph.id().document_key());
            assert_eq!("social", graph.name());
            let expected_edge_defs = vec![EdgeDefinition::new(
                "relation",
                vec!["female".to_owned(), "male".to_owned()],
                vec!["female".to_owned(), "male".to_owned()],
            )];
...
fn replace_edge_definition() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            let replacement_edge_def = EdgeDefinition::new(
                "relation",
...
fn insert_edge() {
    arango_test_with_user_db(
        ...
        |conn, ref mut core| {
            ...
            let person_doc = NewDocument::from(Person {
