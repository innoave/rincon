...
fn create_database_for_default_user() {
    arango_system_db_test(
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_dd1")))
                .unwrap();
        },
    );
}
...
fn create_database_for_one_new_user() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let method = CreateDatabase::with_name_for_users("test_database_d11", vec![new_user]);
            let work = conn.execute(method);
            let created = core.run(work).unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d11")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d1")))
                .unwrap();
        },
    );
}
...
fn create_database_for_two_new_users() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let method = CreateDatabase::with_name_for_users(
                "test_database_d22",
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d22")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d2")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d3")))
                .unwrap();
        },
    );
}
...
fn create_database_for_one_existing_user() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let user = core
                .run(conn.execute(CreateUser::new(new_user.clone())))
                .unwrap();

            assert_eq!("test_user_d4", user.name());

            let method = CreateDatabase::with_name_for_users("test_database_d31", vec![new_user]);
            let work = conn.execute(method);
            let created = core.run(work).unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d31")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d4")))
                .unwrap();
        },
    );
}
...
fn create_database_for_two_existing_users_and_one_new_user() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let user1 = core
                .run(conn.execute(CreateUser::new(new_user1.clone())))
                .unwrap();
            assert_eq!("test_user_d5", user1.name());

            let new_user2 = NewUser::<Empty>::with_name("test_user_d6", "");
            let user2 = core
                .run(conn.execute(CreateUser::new(new_user2.clone())))
                .unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d43")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d5")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d6")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d7")))
                .unwrap();
        },
    );
}
...
fn get_current_database_specific_for_root_user() {
    arango_system_db_test(
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(CreateDatabase::<Empty>::with_name("test_database_d05")))
                .unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d05")))
                .unwrap();
        },
    );
}
...
fn get_current_database_specific_for_user() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let _ =
                core.run(conn.execute(CreateDatabase::with_name_for_users(
                    "test_database_d81",
                    vec![user1],
                ))).unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d81")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d8")))
                .unwrap();
        },
    );
}
...
fn list_databases() {
    arango_system_db_test(
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(CreateDatabase::<Empty>::with_name("test_database_d09")))
                .unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d09")))
                .unwrap();
        },
    );
}
...
fn list_accessible_databases_for_root_user() {
    arango_system_db_test(
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(CreateDatabase::<Empty>::with_name("test_database_d010")))
                .unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d010")))
                .unwrap();
        },
    );
}
...
fn list_accessible_databases_for_test_user() {
    arango_system_db_test(
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d91")))
                .unwrap();
            let _ = core
                .run(conn.execute(DropDatabase::with_name("test_database_d92")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("test_user_d9")))
                .unwrap();
        },
    );
}
