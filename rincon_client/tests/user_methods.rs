...
fn list_databases_for_user_testuser3() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let _ = core
                .run(conn.execute(CreateDatabase::new(new_database1)))
                .unwrap();
            let new_database2 = NewDatabase::new("testbase32", vec![new_user]);
            let _ = core
                .run(conn.execute(CreateDatabase::new(new_database2)))
                .unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("testbase32")))
                .unwrap();
            let _ = core
                .run(conn.execute(DropDatabase::with_name("testbase31")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("testuser3")))
                .unwrap();
        },
    );
}
...
fn get_database_access_level_for_testuser_and_testdatabase() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let _ = core
                .run(conn.execute(CreateDatabase::new(new_database)))
                .unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("testbase41")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("testuser4")))
                .unwrap();
        },
    );
}
...
fn set_database_access_level_for_testuser_and_testdatabase() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let method = SetDatabaseAccessLevel::new(
                "testuser5".into(),
                "testbase51".into(),
                Permission::ReadOnly,
            );
            ...
            let granted =
                core.run(conn.execute(GetDatabaseAccessLevel::new(
                    "testuser5".into(),
                    "testbase51".into(),
                ))).unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("testbase51")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("testuser5")))
                .unwrap();
        },
    );
}
...
fn get_collection_access_level_for_testuser_and_testcollection() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let _ = core
                .run(conn.execute(CreateCollection::with_name("testcollection611")))
                .unwrap();

            let method = GetCollectionAccessLevel::new(
                "testuser6".into(),
                "testbase61".into(),
                "testcollection611".into(),
            );
            let work = conn.execute(method);
            let permission = core.run(work).unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropCollection::with_name("testcollection611")))
                .unwrap();
            let _ = core
                .run(conn.execute(DropDatabase::with_name("testbase61")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("testuser6")))
                .unwrap();
        },
    );
}
...
fn set_collection_access_level_for_testuser_and_testcollection() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let _ = core
                .run(conn.execute(CreateCollection::with_name("testcollection711")))
                .unwrap();

            let method = SetCollectionAccessLevel::new(
                "testuser7".into(),
                "testbase71".into(),
                "testcollection711".into(),
                Permission::ReadOnly,
            );
            let work = conn.execute(method);
            core.run(work).unwrap();
            ...
            let granted =
                core.run(conn.execute(GetCollectionAccessLevel::new(
                    "testuser7".into(),
                    "testbase71".into(),
                    "testcollection711".into(),
                ))).unwrap();
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropCollection::with_name("testcollection711")))
                .unwrap();
            let _ = core
                .run(conn.execute(DropDatabase::with_name("testbase71")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("testuser7")))
                .unwrap();
        },
    );
}
...
fn create_collection_in_database_not_accessible_for_user() {
    arango_system_db_test(
        |conn, ref mut core| {
            ...
            let user_ds = conn.datasource().with_basic_authentication("testuser8", "");
        ...
        |conn, ref mut core| {
            let _ = core
                .run(conn.execute(DropDatabase::with_name("testbase81")))
                .unwrap();
            let _ = core
                .run(conn.execute(DeleteUser::with_name("testuser8")))
                .unwrap();
        },
    );
}
